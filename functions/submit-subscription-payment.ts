...
// El dueño/admin del negocio ya subió el comprobante al bucket privado
...
export default async function handler(req: Request): Promise<Response> {
  ...
  // alguien reporte un pago apuntando al archivo de otro negocio.
  if (!receiptPath.startsWith(`${organizationId}/`)) {
    return jsonResponse({ error: { code: "VALIDATION_ERROR", message: "El comprobante no pertenece a este negocio." } }, 400);
  }
  ...
  try {
    await admin.emails.send({
      to: SUPPORT_EMAIL,
      subject: `Nuevo pago reportado: ${profile?.name ?? organizationId}`,
      html: `<p>El negocio <strong>${profile?.name ?? organizationId}</strong> reportó un pago de suscripción.</p>
        <p>Monto informado: ${amount ?? "no especificado"}</p>
        <p>Nota: ${note ?? "—"}</p>
        <p>Revisalo y confirmalo desde el panel de soporte.</p>`
    });
  } catch (err) {
