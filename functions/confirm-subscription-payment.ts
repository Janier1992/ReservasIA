...
// además reactiva el negocio (si estaba suspendido) y extiende su
// vencimiento — todo en un solo paso ("sin pausas", como pidió el
// usuario), y avisa al negocio por correo.
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (action === "confirm") {
    ...
    if (orgUpdateError) {
      ...
      return jsonResponse({ error: { code: "INTERNAL_ERROR", message: "El pago se marcó confirmado pero no se pudo reactivar el negocio." } }, 500);
    }
  }
  ...
  if (ownerEmail) {
    try {
      const html =
        action === "confirm"
          ? `<p>¡Hola! Confirmamos tu pago de la suscripción de ReservasIA para <strong>${profile?.name ?? "tu negocio"}</strong>.</p>
             <p>Tu servicio queda activo hasta el <strong>${new Date(newExpiresAt!).toLocaleDateString("es-CO")}</strong>.</p>
             <p>Gracias por confiar en nosotros.</p>`
          : `<p>Hola, revisamos el comprobante de pago que enviaste para <strong>${profile?.name ?? "tu negocio"}</strong> y no pudimos confirmarlo.</p>
             <p>Por favor contactanos respondiendo este correo para resolverlo.</p>`;
      await admin.emails.send({
        to: ownerEmail,
        subject: action === "confirm" ? "Pago confirmado — ReservasIA" : "No pudimos confirmar tu pago — ReservasIA",
        html
      });
    } catch (err) {
